...
export class EntregaCokeInvoice extends Document<EntregaCokeInvoiceSchema> {
  ...
  normalize(): this {
    ...
    for (const product of products) {
      const descriptionField = product.find(
        (field) => field.type == EntregaCokeBodyFields.ITEM_DESCRIPCION_PRODUCTO,
      );
  ...
  private inferTotalFactura(): void {
    const products = Utils.groupFields(this.data.detalles);
    const headers = Utils.getFields<EntregaCokeHeaderFields>(this.data.encabezado);

    const calculatedTotal = products.reduce((acc, product) => {
      const fields = Utils.getFields<EntregaCokeBodyFields>(product);
      const valorVenta = this.toNumber(fields[EntregaCokeBodyFields.VALOR_VENTA_ITEM]);
    ...
    const valorTotalFactura = headers[EntregaCokeHeaderFields.VALOR_TOTAL_FACTURA];
    if (
      valorTotalFactura &&
