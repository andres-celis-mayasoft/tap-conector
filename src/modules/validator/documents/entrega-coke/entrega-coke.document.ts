import {
  ENTREGA_COKE_THRESOLDS,
  EntregaCokeBodyFields,
  EntregaCokeHeaderFields,
} from './entrega-coke.fields';
import { EntregaCokeInvoiceSchema } from './entrega-coke.schema';
...
type HeaderField = EntregaCokeInvoiceSchema['encabezado'][number];
type BodyField = EntregaCokeInvoiceSchema['detalles'][number];

export class EntregaCokeInvoice extends Document<EntregaCokeInvoiceSchema> {
  constructor(
    data: EntregaCokeInvoiceSchema,
  ...
  normalize(): this {
    ...
    for (const product of products) {
      const descriptionField = product.find(
        (field) =>
          field.type == EntregaCokeBodyFields.ITEM_DESCRIPCION_PRODUCTO,
      );
      if (descriptionField?.text?.toUpperCase() === 'REDUCCION') {
        const valorField = product.find(
          (field) => field.type === EntregaCokeBodyFields.VALOR_VENTA_ITEM,
        );
        descriptionField.text = descriptionField.text.toUpperCase();
        const valorIBUA = product.find(
          (field) => field.type == EntregaCokeBodyFields.VALOR_IBUA_Y_OTROS,
        );
    ...
    Utils.addMissingFields(
      this.data.detalles,
      Object.values(EntregaCokeBodyFields),
    );
    Utils.parseAndFixNumber(this.data.detalles, [
      EntregaCokeBodyFields.VALOR_VENTA_ITEM,
      EntregaCokeBodyFields.UNIDADES_VENDIDAS,
  ...
  async exclude(): Promise<this> {
    ...
    for (const product of products) {
      const { item_descripcion_producto: descripcion } =
        Utils.getFields<EntregaCokeBodyFields>(product);
  ...
  private inferEncabezado(): void {
    const { fecha_factura, numero_factura, razon_social } =
      Utils.getFields<EntregaCokeHeaderFields>(this.data.encabezado);
  ...
  private async inferDetalles(): Promise<void> {
    const { razon_social } = Utils.getFields<EntregaCokeHeaderFields>(
      this.data.encabezado,
    );
    const products = Utils.groupFields(this.data.detalles);
  ...
  private inferTotalFactura(): void {
    const products = Utils.groupFields(this.data.detalles);
    const headers = Utils.getFields<EntregaCokeHeaderFields>(
      this.data.encabezado,
    );

    const calculatedTotal = products.reduce((acc, product) => {
      const fields = Utils.getFields<EntregaCokeBodyFields>(product);
      const valorVenta = this.toNumber(
        fields[EntregaCokeBodyFields.VALOR_VENTA_ITEM],
      );
      const valorIbua = this.toNumber(
        fields[EntregaCokeBodyFields.VALOR_IBUA_Y_OTROS],
      );
      return acc + valorVenta + valorIbua;
    }, 0);

    const valorTotalFactura =
      headers[EntregaCokeHeaderFields.VALOR_TOTAL_FACTURA];
    if (
      valorTotalFactura &&
  ...
  private guessConfidence(): void {
    Utils.guessConfidence(this.data.encabezado, ENTREGA_COKE_THRESOLDS);
    Utils.guessConfidence(this.data.detalles, ENTREGA_COKE_THRESOLDS);
  }
